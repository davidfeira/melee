...
# Override for PowerPC (mwcc) projects: pulled from melee's build-linux/binutils
_PPC_AS = os.environ.get("PERMUTER_AS")
if _PPC_AS:
    DEFAULT_AS_CMDLINE = shlex.split(_PPC_AS)
...
def import_c_file(
    ...
    for arg in compiler:
        ...
        if arg in ["-D", "-U", "-I"]:
            ...
            include_next = 1
            continue
        # mwcc uses lowercase -i for include paths
        if arg == "-i":
