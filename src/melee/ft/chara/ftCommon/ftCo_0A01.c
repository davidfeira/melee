...
static inline bool ftCo_800A4038_is_small(float x)
{
...
static inline bool ftCo_800A4038_oob(struct Fighter_x1A88_t* x1A88, float x,
                                     float y)
{
    if (x < x1A88->half_width + Stage_GetBlastZoneLeftOffset() ||
        x > Stage_GetBlastZoneRightOffset() - x1A88->half_width ||
        y < x1A88->half_height + Stage_GetBlastZoneBottomOffset() ||
        y > Stage_GetBlastZoneTopOffset() - x1A88->half_height)
    {
        return true;
    }
    return false;
...
static bool ftCo_800A4038(Fighter* fp, bool arg1)
{
    struct Fighter_x1A88_t* x1A88 = &fp->x1A88;
    mp_UnkStruct0* island;
    Vec3 island_pos;
    f32 grav = fp->co_attrs.grav;
    s32 max_t;
    s32 t;
    f32 var_y;
    f32 dx, dy;
    ...
    if (ftCo_800A4038_is_small(grav)) {
        max_t = 1000;
    } else {
        max_t = (s32) (-(-fp->co_attrs.terminal_vel - fp->pos_delta.y) / grav);
    }
         ...
         island = island->next)
    {
        ...
        if (ftCo_800A4038_oob(x1A88, island_pos.x, island_pos.y)) {
            continue;
        }
        if (grCorneria_801E2E50(island->x4)) {
            continue;
        }
        dx = island_pos.x - fp->cur_pos.x;
        if (dx <= 0.0f) {
            continue;
        }
        t = (s32) (dx / fp->co_attrs.air_drift_max);
        if (max_t <= 0) {
            var_y = (fp->pos_delta.y * (f32) t) + fp->cur_pos.y;
        } else if (t < max_t) {
            f32 sq = (f32) t > 0.0f ? sqrtf((f32) t) : (f32) t;
            var_y =
                fp->cur_pos.y + (fp->pos_delta.y * (f32) t -
                                 0.5 * (fp->co_attrs.grav * sq));
        } else {
            f32 sq =
                (f32) max_t > 0.0f ? sqrtf((f32) max_t) : (f32) max_t;
            var_y = fp->cur_pos.y +
                    ((fp->pos_delta.y * (f32) max_t -
                      0.5 * (fp->co_attrs.grav * sq)) -
                     (f32) (t - max_t) * fp->co_attrs.terminal_vel);
        }
        if (arg1) {
            bool good;
            if (var_y + x1A88->x558 < island_pos.y) {
                continue;
            }
            sp70 = -1;
            land_x = 5.0 + island_pos.x;
            hit = mpCheckFloor(land_x, 5.0f + island_pos.y, land_x,
                               island_pos.y - 5.0f, 0.0f, &sp58, &sp70,
                               &sp74, &sp64, -1, -1, -1, NULL, NULL);
            if (hit && ftCo_800A1B38(sp70)) {
                hit = false;
            }
            if (hit && !ftCo_800A4038_oob(x1A88, land_x, island_pos.y)) {
                good = true;
            } else {
                good = false;
            }
            ...
            sp50 = -1;
            land_x = 5.0 + island_pos.x;
            dy = island_pos.y - fp->cur_pos.y;
            dxoff = land_x - fp->cur_pos.x;
            hit = mpCheckFloor(land_x, 5.0f + island_pos.y, land_x,
                               island_pos.y - 5.0f, 0.0f, &sp38, &sp50,
                               &sp54, &sp44, -1, -1, -1, NULL, NULL);
            if (hit && ftCo_800A1B38(sp50)) {
                hit = false;
            }
            if (hit && !ftCo_800A4038_oob(x1A88, land_x, island_pos.y)) {
                good = true;
            ...
            dist_sq = (dxoff * dxoff) + (dy * dy);
            if (dist_sq > 0.0f) {
                dist_sq = sqrtf(dist_sq);
            }
            if (x1A88->x5C > dist_sq) {
                if (x1A88->x60 == 0) {
                    x1A88->x54.x = land_x;
                    x1A88->x54.y = island_pos.y;
                    x1A88->x38 = 5.0f;
